...
public final class ItemDefinitions {
	...
	if (name.contains("sword") || name.contains("dagger")
		|| name.contains("scimitar") || name.contains("whip")
		|| name.contains("spear") || name.contains("mace")
		|| name.contains("battleaxe") || name.contains("staff")
		|| name.contains("Staff") || name.contains("battleaxe")
		|| name.contains("hatchet") || name.contains("pickaxe")
		|| name.contains("axe") || name.contains("wand")
		|| name.contains("katana") || name.contains("Katana")) {
	    return 11;
	}
	if (name.contains("plate") || name.contains("body")
		|| name.contains("apron") || name.contains("chest")
		|| name.contains("top")) {
	    return 3;
	}
	...
	if (name.contains("helm") || name.contains("hat")
		|| name.contains("hood")) {
	    return 0;
	}
	if (name.contains("shield") || name.contains("ket-xil")
		|| name.equalsIgnoreCase("mage book")
    ...
    private short[] oldoriginalTextureColors;
    private short[] oldmodifiedTextureColors;
    private short[] oldoriginalModelColors;
    private short[] oldmodifiedModelColors;
    private int oldInvModelId;
    private int oldMaleEquipModelId3;
    private int oldFemaleEquipModelId3;
    private int oldMaleEquipModelId2;

    private int oldFemaleEquipModelId2;

    private int oldMaleEquipModelId1;

    private int oldFemaleEquipModelId1;
	...
	if (itemRequiriments == null) {
	    ...
	    if (getId() == 18353 || getId() == 18349 || getId() == 18355
		    || getId() == 18355) {
	...
	if (opcode == 1)
		...
		this.clientScriptData = new HashMap<Integer, Object>(length);
	    for (int index = 0; index < length; index++) {
		boolean stringInstance = stream.readUnsignedByte() == 1;
		int key = stream.read24BitInt();
		Object value = stringInstance ? stream.readString() : Integer
			.valueOf(stream.readInt());
		...
		oldModifiedModelTextures[i] = (short) stream
			.readUnsignedShort();
	    }
	} else if (opcode == 253) {
  ...
  public int getPrice() {
        if (GrandExchangePriceLoader.get(id) != null)
	return GrandExchangePriceLoader.get(id).getItemPrice();
        else
            return value;
    }
